import { getDb, saveDb, carListings, carScores, carfaxReports } from '@auto-find/db';
import { eq, and, gt, lt, gte, lte, like, sql, desc, asc, count } from 'drizzle-orm';
import { ListingWithScore } from '@auto-find/shared';
...
export async function getListings(filters: ListingFilters) {
  ...
  const listingsWithScores: ListingWithScore[] = await Promise.all(
...
export async function getListingById(id: number) {
  ...
  return { ...mapListing(result.listing), score: result.score ? mapScore(result.score) : null, carfax: carfax ? mapCarfax(carfax) : null };
}
