...
/**
 * Idempotent schema + reference data bootstrap.
 * Runs once per process on database init so a fresh database (local file or a
 * brand-new Turso instance) is fully usable without a separate migrate step.
 */
...
const SCHEMA_STATEMENTS: string[] = [
  ...
  `CREATE INDEX IF NOT EXISTS listings_make_model ON car_listings(make, model)`,
  `CREATE INDEX IF NOT EXISTS listings_total_cost ON car_listings(total_cost)`,
  `CREATE INDEX IF NOT EXISTS listings_active ON car_listings(is_active)`,
  `CREATE INDEX IF NOT EXISTS listings_vin ON car_listings(vin)`,
  ...
  `CREATE INDEX IF NOT EXISTS scores_composite ON car_scores(composite_score)`,
  `CREATE INDEX IF NOT EXISTS scores_tier ON car_scores(tier)`,
...
export const ratings: Array<{ make: string; model: string; yearStart: number; yearEnd: number; baseScore: number; source: string }> = [
...
/** Seed reference data (sources + reliability ratings) if not already present. */
export async function seedReferenceData(db: any): Promise<void> {
